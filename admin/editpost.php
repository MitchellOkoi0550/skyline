...
require_once "include/editpost_inc.php";
...
<body>
    ...
    <div class="ol-lg-8">
        <div class="row g-0">
            ...
            $record_id = $_GET['post_id']; // Assuming you pass the ID through the URL
            ...
            $post_topic = $record['post_topic'];
            $notes = $record['notes'];
            // Assuming the column name for the image is 'post_image'
            $post_image = $record['post_image']; 
            ?>
            <form action="include/updatepost_inc.php" method="POST" enctype="multipart/form-data" id="postForm">
                <input type="hidden" name="record_id" value="<?php echo $record_id; ?>">
                <div class="row g-3">
                    <div class="form-floating">
                        <select class="form-select" name="floatingSelects" aria-label="Financial Consultancy">
                            <option value="Web Design" <?php if($post_category == "Web Design") echo "selected"; ?>>Web Design</option>
                            <option value="Development" <?php if($post_category == "Development") echo "selected"; ?>>Development</option>
                            <option value="Marketing" <?php if($post_category == "Marketing") echo "selected"; ?>>Marketing</option>
                            <option value="SEO" <?php if($post_category == "SEO") echo "selected"; ?>>SEO</option>
                            <option value="Writing" <?php if($post_category == "Writing") echo "selected"; ?>>Writing</option>
                            <option value="Consulting" <?php if($post_category == "Consulting") echo "selected"; ?>>Consulting</option>
                        </select>
                        <label for="floatingSelect">Select Category</label>
                    </div>
                    <div class="col-12">
                        <div class="form-floating">
                            <input type="text" name="title" class="form-control" id="form-floating-1" required placeholder="Title" value="<?php echo $post_topic; ?>">
                            <label for="form-floating-1">Title</label>
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="form-floating">
                            <textarea class="form-control" id="note" name="note" placeholder="Notes" id="form-floating-4" style="height: 600px"><?php echo $notes; ?></textarea>
                            <label for="form-floating-4">Notes</label>
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="form-floating">
                            <!-- Assuming you want to display the existing image -->
                            <img src="data:image/jpeg;base64,<?php echo base64_encode($post_image); ?>" alt="Post Image">
